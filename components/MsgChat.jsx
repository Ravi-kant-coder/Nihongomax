  ...
  return (
    <div className="">
      {readmsgs.map((dummymsg, index) => (
        <div
          key={dummymsg?.key}
          className="relative group flex items-center py-2 bg-white dark:bg-black rounded-md cursor-pointer hover:bg-gray-200"
        >
          ...
          <div className="w-1/2">
            ...
            <p className="text-xs font-semibold text-black dark:text-gray-400 truncate">
              Sensei Chal chal baliye Sensei Chal chal baliyeSensei Chal chal
              baliyeSensei Chal chal baliye{" "}
            </p>
          </div>
          <div className="absolute bg-gray-200 group-hover:bg-white border border-gray-300 p-2 right-2 w-10 h-10 rounded-full">
            <MoreHorizontal />
          </div>
        </div>
      ))}
      {unreadmsgs.map((dummymsg, index) => (
        <div
          key={dummymsg?.key}
          className="relative group flex items-center py-2 dark:hover:bg-[rgb(30,30,30)] bg-white dark:bg-black rounded-md cursor-pointer hover:bg-gray-200"
        >
          <Avatar className="cursor-pointer h-12 w-12 hover:ring-2 hover:ring-black hover:ring-offset-1 mx-2">
            <AvatarImage src={dummymsg?.imageUrl} className="object-cover" />
            <AvatarFallback className="bg-gray-400 dark:bg-gray-500 text-black">
